...
class ContactManagerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Contact Manager (codsoft) by Anshum")
        self.contacts = []
        self.load_contacts()
        ...
        tk.Label(root, text="Search:", bg="purple", fg="black").grid(row=0, column=0, sticky="w", padx=5)
        tk.Entry(root, textvariable=self.search_var, bg="lavender", fg="black").grid(row=0, column=1, padx=5)
        tk.Button(root, text="Search", command=self.search_contact, bg="black", fg="black").grid(row=0, column=2, padx=5)
        tk.Button(root, text="Add Contact", command=self.add_contact_window, bg="black", fg="black").grid(row=0, column=3, padx=5)

        self.contact_listbox = tk.Listbox(root, width=70, bg="lavender", fg="black")
    ...
    def add_contact_window(self, contact=None, index=None):
        ...
        for i, field in enumerate(fields):
            tk.Label(win, text=field + ":", bg="purple", fg="black").grid(row=i, column=0, padx=5, pady=5, sticky='e')
            entry = tk.Entry(win, width=40, bg="lavender", fg="black")
        ...
        tk.Button(win, text="Save", command=save, bg="black", fg="black").grid(row=5, column=0, columnspan=2, pady=10)
